...
def try_detect_sign(img):
    ...
    res, i = filter_noise(mask_blue, img, 0)
    res, i = filter_noise(mask_red, img, i)
    return res


def filter_noise(mask, img, i):
    ...
    return classify_image(dilate, img, i)


def classify_image(dilate, img, i):
    import os
...
def with_mask(frame, mask2):
    return cv2.bitwise_or(frame, mask2)


def has_similarities(cascade, gray, img, i=0):
